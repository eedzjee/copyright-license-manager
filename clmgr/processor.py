...
class Processor:
    ...
    def _backup_file(self, path):
        return str(path.absolute()) + ".bak"

    def _commit_backup(self, backup_file, path, action):
        os.replace(backup_file, path.absolute())

    def insert_copyright(self, path, ext, offset):
        cfg = self.cfg
        ...
        with open(file=path.absolute(), encoding="utf-8", mode="r") as src_read, open(
            ...
            self._commit_backup(backup_file, path, action="inserted copyright")

    def update_copyright(self, path, ext, offset):
        cfg = self.cfg
        ...
        with open(file=path.absolute(), encoding="utf-8", mode="r") as src_read, open(
            ...
            if cfg["license"]["enabled"]:
                ...
                for x in range(len(lines)):
                    if license_start in lines[x] and x <= self.args.region:
            ...
            changed = not result
            if changed:
                self._commit_backup(backup_file, path, action="update copyright")
            else:
    ...
    def process_lines(self, path, ext, lines):
        add = 0
        upd = 0
        utd = 0
        copyright_start = 3
        ...
        try:
            ...
            if "Copyright" not in start_idx:
                self.insert_copyright(path, ext, offset)
                add += 1
            else:
                if self.update_copyright(path, ext, offset):
                    upd += 1
                else:
                    utd += 1
        except IndexError:
